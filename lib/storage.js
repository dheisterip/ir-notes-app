...
// Tier 1: Personal (per user)
export async function getPersonalTemplates(uid) {
  return (await redisGet('ir_personal:' + uid)) ?? {}
}
export async function setPersonalTemplates(uid, templates) {
  await redisSet('ir_personal:' + uid, templates)
}

// Tier 2: Universal (admin-managed)
...
// Tier 3: Community (future - empty for now)
export async function getCommunityTemplates() { return {} }

// Effective templates: personal overrides universal overrides defaults
export async function getEffectiveTemplates(uid) {
  const [universal, personal] = await Promise.all([getTemplates(), getPersonalTemplates(uid)])
  return Object.assign({}, universal, personal)
}

// ── User preferences ──────────────────────────────────────────────────────
export async function getUserPrefs(uid) {
  return (await redisGet('ir_prefs:' + uid)) ?? {}
}
export async function setUserPrefs(uid, prefs) {
  await redisSet('ir_prefs:' + uid, prefs)
}
  ...
  try {
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: '<h2>IR Notes Usage Alert</h2><p>Monthly spend exceeded $' + settings.threshold + '.</p><p><strong>Total: $' + totalCost.toFixed(2) + '</strong></p>'
      })
    })
