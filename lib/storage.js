  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [settings.email],
        subject: 'IR Notes: $' + settings.threshold + ' usage alert — ' + month,
        html: '<h2>IR Notes Usage Alert</h2><p>Monthly API spend has exceeded $' + settings.threshold + '.</p><p><strong>Current total: $' + totalCost.toFixed(2) + '</strong></p><p>Log in to the admin dashboard for details.</p>'
      })
    })
